...
def blockUNet(in_c, out_c, name, transposed=False, bn=True, relu=True, dropout=False):
    ...
    if not transposed:
        ...
        block.add_module(
            "%s.tconv" % name, nn.ConvTranspose2d(in_c, out_c, 4, 2, 1, bias=False)
        )
    if bn:
...
class D(nn.Module):
    def __init__(self, nc, nf):
        ...
        main.add_module(
            name,
            blockUNet(
                nf, nf * 2, name, transposed=False, bn=True, relu=False, dropout=False
            ),
        )
...
class G(nn.Module):
    def __init__(self, input_nc, output_nc, nf):
        ...
        name = "layer%d" % layer_idx
        layer3 = blockUNet(
            nf * 2, nf * 4, name, transposed=False, bn=True, relu=False, dropout=False
        )
        # input is 32
        layer_idx += 1
        name = "layer%d" % layer_idx
        layer4 = blockUNet(
            nf * 4, nf * 8, name, transposed=False, bn=True, relu=False, dropout=False
        )
        # input is 16
        layer_idx += 1
        name = "layer%d" % layer_idx
        layer5 = blockUNet(
            nf * 8, nf * 8, name, transposed=False, bn=True, relu=False, dropout=False
        )
        # input is 8
        layer_idx += 1
        name = "layer%d" % layer_idx
        ...
        name = "layer%d" % layer_idx
        layer8 = blockUNet(
            nf * 8, nf * 8, name, transposed=False, bn=False, relu=False, dropout=False
        )
        ...
        d_inc = nf * 8
        dlayer8 = blockUNet(
            d_inc, nf * 8, name, transposed=True, bn=True, relu=True, dropout=True
        )
        ...
        d_inc = nf * 8 * 2
        dlayer5 = blockUNet(
            d_inc, nf * 8, name, transposed=True, bn=True, relu=True, dropout=False
        )
        # input is 16
        layer_idx -= 1
        name = "dlayer%d" % layer_idx
        d_inc = nf * 8 * 2
        dlayer4 = blockUNet(
            d_inc, nf * 4, name, transposed=True, bn=True, relu=True, dropout=False
        )
        # input is 32
        layer_idx -= 1
        name = "dlayer%d" % layer_idx
        d_inc = nf * 4 * 2
        dlayer3 = blockUNet(
            d_inc, nf * 2, name, transposed=True, bn=True, relu=True, dropout=False
        )
        # input is 64
        layer_idx -= 1
        name = "dlayer%d" % layer_idx
        d_inc = nf * 2 * 2
        dlayer2 = blockUNet(
            d_inc, nf, name, transposed=True, bn=True, relu=True, dropout=False
        )
        # input is 128
        layer_idx -= 1
        name = "dlayer%d" % layer_idx
        ...
        dlayer1.add_module(
            "%s.tconv" % name, nn.ConvTranspose2d(d_inc, output_nc, 4, 2, 1, bias=False)
        )
